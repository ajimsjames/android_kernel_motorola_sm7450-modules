#include <linux/module.h>
#define INCLUDE_VERMAGIC
#include <linux/build-salt.h>
#include <linux/vermagic.h>
#include <linux/compiler.h>

BUILD_SALT;

MODULE_INFO(vermagic, VERMAGIC_STRING);
MODULE_INFO(name, KBUILD_MODNAME);

__visible struct module __this_module
__section(".gnu.linkonce.this_module") = {
	.name = KBUILD_MODNAME,
	.init = init_module,
#ifdef CONFIG_MODULE_UNLOAD
	.exit = cleanup_module,
#endif
	.arch = MODULE_ARCH_INIT,
};

#ifdef CONFIG_RETPOLINE
MODULE_INFO(retpoline, "Y");
#endif

static const struct modversion_info ____versions[]
__used __section("__versions") = {
	{ 0xe3c515c, "module_layout" },
	{ 0xfc52abc7, "qcom_scm_pas_shutdown" },
	{ 0x9e9fdd9d, "memunmap" },
	{ 0xdd7e3192, "qcom_scm_pas_auth_and_reset" },
	{ 0x5c33ec1f, "qcom_mdt_load" },
	{ 0x4d924f20, "memremap" },
	{ 0xe8a3861c, "qcom_mdt_get_size" },
	{ 0xa45b3c60, "request_firmware" },
	{ 0xc6d09aa9, "release_firmware" },
	{ 0xb23a57e7, "of_address_to_resource" },
	{ 0xc094b1d, "synx_wait" },
	{ 0x9a5e29ff, "synx_signal" },
	{ 0x23465350, "synx_release" },
	{ 0xcd56f629, "synx_import" },
	{ 0xb40c7137, "synx_get_status" },
	{ 0x19d93bd8, "synx_uninitialize" },
	{ 0x8f16b6a4, "synx_initialize" },
	{ 0x479c3c86, "find_next_zero_bit" },
	{ 0xb6c02e0d, "fput" },
	{ 0x2469810f, "__rcu_read_unlock" },
	{ 0x8d522714, "__rcu_read_lock" },
	{ 0x5464fe2f, "__put_task_struct" },
	{ 0x632393da, "rpmsg_send" },
	{ 0xbee58674, "unregister_rpmsg_driver" },
	{ 0x93d6dd8c, "complete_all" },
	{ 0xcdf8a885, "fastrpc_driver_unregister" },
	{ 0xa3781226, "fastrpc_driver_register" },
	{ 0xa9894560, "get_pid_task" },
	{ 0xa269247e, "find_get_pid" },
	{ 0x2e3bcce2, "wait_for_completion_interruptible" },
	{ 0xbcd37b28, "__register_rpmsg_driver" },
	{ 0x12b049fb, "fastrpc_driver_invoke" },
	{ 0x398b9914, "hyp_assign_phys" },
	{ 0x6d5659c, "icc_set_bw" },
	{ 0x2e1ca751, "clk_put" },
	{ 0x51ec673b, "clk_get" },
	{ 0xb6e6d99d, "clk_disable" },
	{ 0xb077e70a, "clk_unprepare" },
	{ 0x815588a6, "clk_enable" },
	{ 0x7c9a7371, "clk_prepare" },
	{ 0x43f81957, "clk_round_rate" },
	{ 0x76d9b876, "clk_set_rate" },
	{ 0xf77f51e3, "mmrm_client_set_value_in_range" },
	{ 0x15e4a2d8, "mmrm_client_deregister" },
	{ 0xc6a4a872, "__clk_is_enabled" },
	{ 0xbbd8dc5d, "mmrm_client_register" },
	{ 0xde9150a8, "mmrm_client_check_scaling_supported" },
	{ 0x176e1ed0, "kmem_cache_alloc" },
	{ 0xc30b6f92, "dma_unmap_page_attrs" },
	{ 0x42160169, "flush_workqueue" },
	{ 0x78c7940d, "mutex_trylock" },
	{ 0xffb9e063, "dev_pm_qos_remove_request" },
	{ 0x9cb025a4, "dma_free_attrs" },
	{ 0x874f1c97, "dev_driver_string" },
	{ 0xa1cf65e0, "dev_pm_qos_add_request" },
	{ 0x25a78573, "get_cpu_device" },
	{ 0x20f420a, "dma_map_page_attrs" },
	{ 0xc31db0ce, "is_vmalloc_addr" },
	{ 0xff8e54c1, "dma_alloc_attrs" },
	{ 0x3e9a126c, "iommu_unmap" },
	{ 0x965874eb, "iommu_map" },
	{ 0xcf51ae91, "llcc_slice_putd" },
	{ 0x8e19f137, "pm_relax" },
	{ 0x2dc32291, "llcc_slice_getd" },
	{ 0x1533f254, "icc_put" },
	{ 0x56470118, "__warn_printk" },
	{ 0x2e4cb387, "icc_get" },
	{ 0xc253ae06, "__devm_reset_control_get" },
	{ 0x9dca9a69, "regulator_put" },
	{ 0x719ce0bf, "regulator_get" },
	{ 0x279b23ae, "pm_stay_awake" },
	{ 0x7e5ade52, "llcc_slice_deactivate" },
	{ 0xfe916dc6, "hex_dump_to_buffer" },
	{ 0x39f96177, "regulator_enable" },
	{ 0x66b4cc41, "kmemdup" },
	{ 0x6b7609b9, "llcc_slice_activate" },
	{ 0x7c8b8ace, "dev_pm_qos_update_request" },
	{ 0xde7ece30, "__cpu_possible_mask" },
	{ 0x9b128a66, "qcom_scm_set_remote_state" },
	{ 0xfcec0987, "enable_irq" },
	{ 0xbf57e89e, "queue_delayed_work_on" },
	{ 0xda571594, "cancel_delayed_work" },
	{ 0xa07a37f0, "memchr" },
	{ 0x1f7b7ec7, "regulator_is_enabled" },
	{ 0x82584bd0, "regulator_set_mode" },
	{ 0x297579ba, "regulator_disable" },
	{ 0x31dfd5cd, "__log_write_mmio" },
	{ 0x95575c33, "__tracepoint_rwmmio_write" },
	{ 0x6980ea4b, "__log_post_read_mmio" },
	{ 0xcf1211a8, "__log_read_mmio" },
	{ 0x19ebf04e, "__tracepoint_rwmmio_post_read" },
	{ 0xa035d76e, "__tracepoint_rwmmio_read" },
	{ 0x3d6dd0aa, "mutex_is_locked" },
	{ 0xb2210d64, "reset_control_deassert" },
	{ 0x6c4b6684, "reset_control_assert" },
	{ 0xdf9208c0, "alloc_workqueue" },
	{ 0x27bbf221, "disable_irq_nosync" },
	{ 0x2072ee9b, "request_threaded_irq" },
	{ 0x1dc7fa5f, "devm_ioremap" },
	{ 0xedc03953, "iounmap" },
	{ 0xc1514a3b, "free_irq" },
	{ 0x8c03d20c, "destroy_workqueue" },
	{ 0x817876ce, "msm_minidump_remove_region" },
	{ 0x5bb305e8, "msm_minidump_add_region" },
	{ 0x584e21a2, "msm_minidump_enabled" },
	{ 0x43c31ab3, "of_reserved_mem_device_init_by_idx" },
	{ 0x35b93f80, "devm_kstrdup" },
	{ 0xb1e12d81, "krealloc" },
	{ 0x606751d6, "iommu_set_fault_handler" },
	{ 0x33ad8434, "iommu_get_domain_for_dev" },
	{ 0x9cf0dacc, "of_property_read_string" },
	{ 0x6b9bd4b2, "devm_kfree" },
	{ 0xb7c0f443, "sort" },
	{ 0x5792f848, "strlcpy" },
	{ 0x25306f85, "of_parse_phandle" },
	{ 0xc310b981, "strnstr" },
	{ 0x98cf60b3, "strlen" },
	{ 0x7c02aadb, "of_property_read_string_helper" },
	{ 0xe4a318dd, "platform_get_resource" },
	{ 0x7281ec27, "cpu_topology" },
	{ 0xb9a35017, "devm_kmalloc" },
	{ 0xb602cbbc, "of_get_property" },
	{ 0x9c3d2b68, "of_find_property" },
	{ 0xb9fb56dd, "simple_attr_release" },
	{ 0x66694ffc, "debugfs_attr_write" },
	{ 0x6d0772a4, "debugfs_attr_read" },
	{ 0x39c74d8, "no_llseek" },
	{ 0xaf507de1, "__arch_copy_from_user" },
	{ 0x88db9f48, "__check_object_size" },
	{ 0xaa0c318b, "vscnprintf" },
	{ 0x619cb7dd, "simple_read_from_buffer" },
	{ 0xddf6ad7a, "completion_done" },
	{ 0x45139159, "simple_attr_open" },
	{ 0x591ee2bf, "debugfs_create_file" },
	{ 0x611daf09, "debugfs_create_bool" },
	{ 0x3acd5f9c, "debugfs_create_u32" },
	{ 0x33172d60, "debugfs_create_x32" },
	{ 0xa33b2b66, "debugfs_create_dir" },
	{ 0x6be11a94, "dma_map_resource" },
	{ 0x967d86b, "dma_buf_end_cpu_access_partial" },
	{ 0x6632f3ad, "dma_buf_begin_cpu_access_partial" },
	{ 0xea2e69ac, "dma_buf_vunmap" },
	{ 0x7ca75f11, "dma_buf_end_cpu_access" },
	{ 0xf20d6f2e, "dma_buf_vmap" },
	{ 0xf936c47c, "dma_buf_begin_cpu_access" },
	{ 0xa2b09770, "mem_buf_lend" },
	{ 0x962bf470, "dma_heap_buffer_alloc" },
	{ 0xeff75c0b, "dma_heap_find" },
	{ 0x9688de8b, "memstart_addr" },
	{ 0xe3a8955e, "dma_buf_detach" },
	{ 0xa42eea7f, "dma_buf_unmap_attachment" },
	{ 0xc967b12b, "dma_buf_map_attachment" },
	{ 0x70312a2b, "dma_buf_attach" },
	{ 0xefb872a1, "mem_buf_dma_buf_copy_vmperm" },
	{ 0x27b5a18c, "dma_heap_buffer_free" },
	{ 0xca311a4a, "dma_buf_get" },
	{ 0x1000e51, "schedule" },
	{ 0x952664c5, "do_exit" },
	{ 0x8ddd8aad, "schedule_timeout" },
	{ 0x92540fbf, "finish_wait" },
	{ 0x8c26d495, "prepare_to_wait_event" },
	{ 0xfe487975, "init_wait_entry" },
	{ 0xd1ce2398, "wake_up_process" },
	{ 0x4ec0b590, "kthread_create_on_node" },
	{ 0xeb233a45, "__kmalloc" },
	{ 0xb5b54b34, "_raw_spin_unlock" },
	{ 0x7baf2a33, "kmem_cache_free" },
	{ 0xba8fbd64, "_raw_spin_lock" },
	{ 0xe1537255, "__list_del_entry_valid" },
	{ 0xd9a5ea54, "__init_waitqueue_head" },
	{ 0x12a38747, "usleep_range" },
	{ 0x732ac580, "queue_work_on" },
	{ 0x2d3385d3, "system_wq" },
	{ 0xdcb764ad, "memset" },
	{ 0x3eeb2322, "__wake_up" },
	{ 0x4829a47e, "memcpy" },
	{ 0xa6257a2f, "complete" },
	{ 0x4a3ad70e, "wait_for_completion_timeout" },
	{ 0x7f02188f, "__msecs_to_jiffies" },
	{ 0xc56a41e6, "vabits_actual" },
	{ 0x296695f, "refcount_warn_saturate" },
	{ 0xe2d5255a, "strcmp" },
	{ 0x39bfa514, "of_match_node" },
	{ 0xfec2174f, "of_property_read_variable_u32_array" },
	{ 0xc1b6ddce, "of_find_node_opts_by_path" },
	{ 0x8b9f70c7, "cpu_hwcaps" },
	{ 0x4b0a3f52, "gic_nonsecure_priorities" },
	{ 0xec2fc692, "cpu_hwcap_keys" },
	{ 0x14b89635, "arm64_const_caps_ready" },
	{ 0x96848186, "scnprintf" },
	{ 0x2276db98, "kstrtoint" },
	{ 0x98a9d10c, "__stack_chk_fail" },
	{ 0x3c80c06c, "kstrtoull" },
	{ 0x656e4a6e, "snprintf" },
	{ 0xd35cce70, "_raw_spin_unlock_irqrestore" },
	{ 0x34db050b, "_raw_spin_lock_irqsave" },
	{ 0x2115221d, "sysfs_remove_group" },
	{ 0x6091b333, "unregister_chrdev_region" },
	{ 0x91eaa13e, "class_destroy" },
	{ 0xadfc90e, "device_destroy" },
	{ 0xcbdd06ee, "cdev_del" },
	{ 0x742586a2, "of_platform_populate" },
	{ 0x68f31cbd, "__list_add_valid" },
	{ 0x7a268829, "sysfs_create_group" },
	{ 0xe8b268ae, "mutex_unlock" },
	{ 0xeb9065d9, "mutex_lock" },
	{ 0x4305d412, "cdev_add" },
	{ 0x77f94169, "cdev_init" },
	{ 0xf987a6, "device_create" },
	{ 0xf4d3debf, "__class_create" },
	{ 0xe3ec2f2b, "alloc_chrdev_region" },
	{ 0x79cf5987, "init_timer_key" },
	{ 0x253af43d, "delayed_work_timer_fn" },
	{ 0x608741b5, "__init_swait_queue_head" },
	{ 0xb8b9f817, "kmalloc_order_trace" },
	{ 0xe3463541, "of_device_is_compatible" },
	{ 0x58ba3d68, "debugfs_remove" },
	{ 0x4c0b8512, "platform_driver_unregister" },
	{ 0x7ff3ed10, "kmem_cache_destroy" },
	{ 0x9ffce5b0, "kmem_cache_create" },
	{ 0xc5850110, "printk" },
	{ 0x37a0cba, "kfree" },
	{ 0xedf3b609, "__platform_driver_register" },
	{ 0x574add77, "__mutex_init" },
	{ 0xbaa6702f, "kmem_cache_alloc_trace" },
	{ 0xce7f4b8f, "kmalloc_caches" },
};

MODULE_INFO(depends, "qcom-scm,mdt_loader,synx-driver,frpc-adsprpc,secure_buffer,msm-mmrm,llcc-qcom,minidump,mem_buf_dev");

MODULE_ALIAS("of:N*T*Cqcom,msm-cvp");
MODULE_ALIAS("of:N*T*Cqcom,msm-cvpC*");
MODULE_ALIAS("of:N*T*Cqcom,msm-cvp,context-bank");
MODULE_ALIAS("of:N*T*Cqcom,msm-cvp,context-bankC*");
MODULE_ALIAS("of:N*T*Cqcom,msm-cvp,bus");
MODULE_ALIAS("of:N*T*Cqcom,msm-cvp,busC*");
MODULE_ALIAS("of:N*T*Cqcom,msm-cvp,mem-cdsp");
MODULE_ALIAS("of:N*T*Cqcom,msm-cvp,mem-cdspC*");
MODULE_ALIAS("of:N*T*Cqcom,waipio-cvp");
MODULE_ALIAS("of:N*T*Cqcom,waipio-cvpC*");
MODULE_ALIAS("of:N*T*Cqcom,kalama-cvp");
MODULE_ALIAS("of:N*T*Cqcom,kalama-cvpC*");

MODULE_INFO(scmversion, "g2806f02f43ff");
